...
class VideoRepository(private val context: Context) {
    ...
    val videoUris: Flow<List<String>> = context.dataStore.data.map { prefs ->
        deserialize(prefs[VIDEO_URIS_KEY] ?: "")
    }
    ...
    val selectedVideos: Flow<Set<String>> = context.dataStore.data.map { prefs ->
        parseCommaSet(prefs[SELECTED_VIDEOS_KEY] ?: "")
    }
    ...
    suspend fun addVideoUri(uri: String) {
        context.dataStore.edit { prefs ->
            val existing = deserialize(prefs[VIDEO_URIS_KEY] ?: "").toMutableList()
            if (uri !in existing) {
                existing.add(uri)
    ...
    suspend fun removeVideoUri(uri: String) {
        context.dataStore.edit { prefs ->
            val uris = deserialize(prefs[VIDEO_URIS_KEY] ?: "").filter { it != uri }
    ...
    suspend fun toggleVideoSelection(uri: String) {
        context.dataStore.edit { prefs ->
            val current = parseCommaSet(prefs[SELECTED_VIDEOS_KEY] ?: "")
